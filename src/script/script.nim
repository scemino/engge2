...
proc register_gameconstants*(v: HSQUIRRELVM) =
  sqBind(v):
    const
      ...
      EX_ALLOW_SAVEGAMES = EX_ALLOW_SAVEGAMES
      EX_POP_CHARACTER_SELECTION = EX_POP_CHARACTER_SELECTION
      EX_CAMERA_TRACKING = EX_CAMERA_TRACKING
      EX_BUTTON_HOVER_SOUND = EX_BUTTON_HOVER_SOUND
      EX_RESTART = EX_RESTART
      EX_IDLE_TIME = EX_IDLE_TIME
      EX_AUTOSAVE = EX_AUTOSAVE
      EX_AUTOSAVE_STATE = EX_AUTOSAVE_STATE
      EX_DISABLE_SAVESYSTEM = EX_DISABLE_SAVESYSTEM
      EX_SHOW_OPTIONS = 11
      EX_OPTIONS_MUSIC = 12
